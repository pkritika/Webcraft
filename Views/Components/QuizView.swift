...
struct QuizView: View {
        ...
        VStack(spacing: 24) {
            ...
            VStack(spacing: 12) {
                ForEach(0..<quiz.options.count, id: \.self) { index in
                    ...
                    .accessibilityLabel(quiz.options[index])
                    .accessibilityHint(getAccessibilityHint(for: index))
    ...
    private func getAccessibilityHint(for index: Int) -> String {
        if hasSubmitted {
            if index == quiz.correctAnswerIndex {
        ...
        return selectedIndex == index ? "Selected. Double tap to deselect" : "Double tap to select this answer"
    }
}
